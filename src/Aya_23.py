...
# =================================================================
# GIGA-SCALE NEUROCARTOGRAPHY: 6 BENCHMARKS (DEEP STRUCTURAL MAPPING)
# =================================================================
MODEL_ID = "CohereForAI/aya-23-35B" # <--- Change this to Nemotron for your 2nd run
SHORT_NAME = MODEL_ID.split('/')[-1].replace("-", "_")

LAYER_IDX = 22         # Deep Semantic Layer
N_CONCEPTS = 1000      # Giga-Resolution 
MAX_SAMPLES = 2000     # 12,000 total benchmark inferences

print(f"🌌 INITIALIZING GIGA-SCALE STUDY: {MODEL_ID}")

# 1. LOAD MODEL
tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
model = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.bfloat16, device_map="auto")

# 2. STRUCTURAL WEIGHT EXTRACTION (SVD)
print(f"\n[1/5] Extracting Structural Knowledge Skeleton (1000 Nodes)...")
with torch.no_grad():
    W = model.model.layers[LAYER_IDX].mlp.up_proj.weight.detach().float()
    U, S, Vh = torch.linalg.svd(W, full_matrices=False)
    concepts = Vh[:N_CONCEPTS].cpu() 
    # Capture Singular Values (Mathematical Importance) for the deep CSV
...
for name, (path, conf, split) in benchmarks.items():
    print(f"\n   -> BENCHMARK: {name} (Gathering Neural Signatures)...")
    try:
        ...
        handle = model.model.layers[LAYER_IDX].mlp.register_forward_hook(get_deep_hook(name))
        
        for row in tqdm(subset_data, leave=False):
            # Universal Multi-Benchmark Text Extractor
...
# 4. DEEP STRUCTURAL NORMALIZATION
print("\n[3/5] Normalizing multi-dimensional cognitive data...")
voting = {}
for b in benchmarks:
...
# Vibrant scientific colors for 6 categories
colors_map = {
    "TruthfulQA": "#2ecc71",   # Green
    "ETHICS": "#e74c3c",       # Red
    "MMLU": "#3498db",         # Blue
    "Logic": "#9b59b6",        # Purple
    "GPQA_PhD": "#f1c40f",     # Gold (High-level abstract reasoning)
    "RedTeaming": "#34495e"    # Dark Slate (Safety/Adversarial guardrails)
}

# 5. STRUCTURAL LABELING & DEEP CSV EXPORT
print("\n[4/5] Labeling 1000 Nodes and Generating Deep CSV...")
for i in tqdm(range(N_CONCEPTS), desc="Extracting Conceptual DNA"):
    ...
    # Building the DEEP CSV structural row
    row_data = {
        "node_id": i,
...
# 6. GIGA-SCALE GRAPH VISUALIZATION
print("\n[5/5] Generating Giga-Scale Visual Knowledge Map...")
...
# Sparsity threshold for clarity on 1000 nodes (99th percentile)
thresh = np.percentile(sim, 99.2)
...
nx.draw_networkx_nodes(G, pos, node_size=120, node_color=node_colors, alpha=0.9, edgecolors='black', linewidths=0.1)
nx.draw_networkx_edges(G, pos, alpha=0.03, edge_color='gray')
nx.draw_networkx_labels(G, pos, labels={d['node_id']: d['structural_label'] for d in node_data}, font_size=4, font_color='black')

re = [plt.Line2D([0], [0], marker='o', color='w', label=k, markerfacecolor=v, markersize=24) for k, v in colors_map.items()]
plt.legend(handles=re, loc='upper right', fontsize=24, framealpha=0.9, title="Cognitive Domain", title_fontsize=30)

plt.title(f"GIGA-MAP (1000 Nodes): {MODEL_ID}\n(6 Domains including PhD-GPQA & Red-Teaming)", fontsize=45)
plt.axis('off')
map_file = f"GIGA_MAP_{SHORT_NAME}.png"
plt.savefig(map_file, dpi=600, bbox_inches='tight') 

print(f"\n✅ REVOLUTIONARY STUDY COMPLETE FOR {MODEL_ID}!")
print(f"Deep Structural CSV: {csv_file}")
print(f"8K Visual Atlas: {map_file}")